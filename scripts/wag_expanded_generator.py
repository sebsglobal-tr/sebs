...
<p>Bu terminal blokları <strong>öğretim amaçlıdır</strong>. Çıktıları kanıt paketine alırken URL, token ve kişisel veriyi maskeleyin.</p>
{terminal("lab-01-http.sh — TLS ve başlık (YOUR-LAB)", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">curl -sI https://YOUR-LAB/ | sed -n '1,25p'</span>
<span class="term-output">HTTP/2 200</span>
...
<span class="term-comment"># Sertifika süresi ve konu: yanlış SAN / süresi dolmuş sertifika keşfi.</span>""")}
{terminal("lab-02-dns.sh — çözümleme ve kayıt türleri", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">dig +short A YOUR-LAB</span>
<span class="term-output">203.0.113.10</span>
<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">dig +short CNAME www.YOUR-LAB</span>
<span class="term-output">cdn.example.net.</span>
<span class="term-comment"># Subdomain takeover hipotezi: CNAME hedefi boşta mı? (yetkili envanter ile)</span>""")}
{terminal("lab-03-json-api.sh — REST çağrısı ve jq", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">curl -sS -H 'Authorization: Bearer YOUR_TOKEN' \\</span>
<span class="term-cmd">  'https://YOUR-LAB/api/v1/me' | jq '{id, role, email}'</span>
<span class="term-output">{ "id": 12, "role": "user", "email": "u@example.com" }</span>
<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">curl -sS -o /dev/null -w '%{http_code} %{time_total}\\n' -X PATCH \\</span>
<span class="term-cmd">  -H 'Content-Type: application/json' -d '{{"role":"admin"}}' \\</span>
<span class="term-cmd">  'https://YOUR-LAB/api/v1/me'</span>
<span class="term-output">403 0.04</span>
<span class="term-comment"># Mass assignment denemesi: 403 beklenir; 200 ise bulgu adayı.</span>""")}
{terminal("lab-04-jwt-decode.sh — başlangıç (imza doğrulamadan)", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">JWT='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.dozdj93n...'</span>
<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">python3 -c \"import sys,base64,json; p=sys.argv[1].split('.'); print(json.dumps(json.loads(base64.urlsafe_b64decode(p[0]+'==')), indent=2)); print(json.dumps(json.loads(base64.urlsafe_b64decode(p[1]+'==')), indent=2))\" \"$JWT\"</span>
<span class="term-output">{{ \"alg\": \"HS256\", \"typ\": \"JWT\" }}</span>
<span class="term-output">{{ \"sub\": \"1234567890\" }}</span>
<span class="term-comment"># 'alg':'none' veya anahtar confüzyonu labda güvenli ortamda test edilir; üretimde asla.</span>""")}
{terminal("lab-05-log-grep.sh — access logda şüpheli örüntü", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">grep -E \"(%27|%22|union|select|script|\\.\\./)\" access.log | tail -n 5</span>
<span class="term-output">GET /search?q=%3Cscript%3Ealert(1)%3C%2Fscript%3E HTTP/1.1\" 200</span>
<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">awk '$9 ~ /^4/ {{print}}' access.log | wc -l</span>
<span class="term-output">842</span>
<span class="term-comment"># 4xx artışı: keşif veya yanlış yetki politikası sinyali.</span>""")}
...
{terminal("lab-06-curl-sqli-xss.sh — yansıtma kontrolü (YOUR-LAB)", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">curl -sG 'https://YOUR-LAB/search' --data-urlencode \"q=test'\" | head -c 400</span>
<span class="term-output">...test'...</span>
<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">curl -sG 'https://YOUR-LAB/comment' --data-urlencode 'text=<svg/onload=alert(1)>' | head -c 400</span>
<span class="term-comment"># Çıktıda decode edilmiş script görünüyorsa encode eksikliği vardır.</span>""")}
{terminal("lab-07-sqlmap.sh — yalnız yetkili lab (örnek invokasyon)", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">sqlmap -u 'https://YOUR-LAB/vuln.php?id=1' --batch --risk=1 --level=1 --flush-session</span>
<span class="term-output">[INFO] testing connection to the target URL</span>
<span class="term-output">[INFO] testing if the target URL content is stable</span>
<span class="term-comment"># --risk/--level artışı agresiflik getirir; yalnızca RoE yazılı labda.</span>
<span class="term-comment"># Üretim veya kapsam dışı host: kesinlikle kullanmayın.</span>""")}
{terminal("lab-08-ffuf.sh — dizin/uç keşfi (rate limit ile)", """<span class="term-prompt">student@lab:~$</span> <span class="term-cmd">ffuf -w /usr/share/wordlists/dirb/common.txt -u https://YOUR-LAB/FUZZ \\</span>
<span class="term-cmd">  -mc 200,204,301,302,403 -rate 20 -t 2</span>
<span class="term-output">admin                 [Status: 302, Size: 0]</span>
<span class="term-output">.git/HEAD             [Status: 403, Size: 120]</span>
<span class="term-comment"># ffuf/gobuster/dirb: yalnızca size ait lab; rate ve thread düşük tutun.</span>""")}
""" + quiz(
    [

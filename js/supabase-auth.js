...
class SupabaseAuthSystem {
  ...
  async updateNavigation() {
    ...
    if (typeof window.sebsApplySignupNavCta === 'function') {
      window.sebsApplySignupNavCta(!!(this.isLoggedIn && this.user), this.user || null);
    }
  }
...
document.addEventListener('DOMContentLoaded', async function() {
  try {
    ...
    // Network durumu dinleme — offline banner + yeniden bağlanınca session kontrolü
    if (typeof window.NetworkStatus !== 'undefined') {
      window.NetworkStatus.onChange(function (online) {
        if (typeof window.OfflineOverlay !== 'undefined') {
          window.OfflineOverlay.toggle(!online);
        }
        if (online && supabaseAuthSystem) {
          console.log('🌐 Network geri geldi, session yenileniyor...');
          supabaseAuthSystem.checkSession().catch(function () {});
          supabaseAuthSystem.updateNavigation().catch(function () {});
        }
      });
      // Başlangıç durumu
      if (!window.NetworkStatus.isOnline() && typeof window.OfflineOverlay !== 'undefined') {
        window.OfflineOverlay.toggle(true);
      }
    }
  } catch (error) {

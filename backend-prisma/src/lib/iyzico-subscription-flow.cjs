...
function isSubscriptionWebhookEvent(payload) {
    ...
    if (/^subscription_/i.test(eventType)) return true;
    return Boolean(payload.subscriptionReferenceCode) && !payload.token;
}

async function handleSubscriptionWebhook(pool, payload, signature) {
    await ensureSubscriptionTables(pool);

    if (signature) {
        const sigOk = verifyHppWebhookSignature(payload, signature);
        if (!sigOk) {
            throw Object.assign(new Error('Webhook imzası geçersiz'), { code: 'WEBHOOK_SIGNATURE_INVALID' });
        }
    }

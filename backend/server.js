...
/**
 * Health endpoint — tüm middleware'den ÖNCE tanımlanır, böylece
 * rate limiter / CORS / auth ile asla engellenmez.
 *
 * Render health check, UptimeRobot (HEAD) ve GitHub Actions keep-alive
 * buraya güvenir. Her zaman hızlı yanıt verir, veritabanı sorgusu içermez.
 * Cache-Control: no-store — proxy/CDN yanıtı önbelleğe almamalı.
 */
    ...
    contentSecurityPolicy: {
        ...
        directives: {
            ...
            scriptSrc: [
                ...
                "'unsafe-eval'", // Tailwind CSS CDN için gerekli (mümkünse build-time Tailwind'e geçip kaldırın)
...
const authenticateToken = (req, res, next) => {
    ...
    // Fallback: HttpOnly cookie'den token oku
    if (!token && req.cookies && req.cookies.sebs_at) {
        token = decodeURIComponent(req.cookies.sebs_at);
    }
    ...
    try {
        ...
        if (token) {
            try {
                ...
                // user_package_purchases'dan da satın alımları kontrol et
                try {
                    ...
                    for (const p of pkgResult.rows) {
                        if (!userPurchases.some(x => x.category === p.category && x.level === p.level)) {
                            userPurchases.push(p);
                        }
                    }
                } catch (err) {
                    console.log('Could not fetch package purchases:', err.message);
                }
...
/* ============================================
 * PURCHASE NOTE
 * purchases ve user_package_purchases paralel çalışır.
 * Kod her ikisine de yazar, okurken merge eder.
 * Gelecekte user_package_purchases kaldırılmalıdır.
 * ============================================ */
...
const CERTIFICATE_CATEGORIES = new Set(['cybersecurity', 'cloud', 'data-science']);
...
app.get('/api/certificates/check/:category', authenticateToken, async (req, res) => {
    try {
        ...
        const categoryTitles = { cybersecurity: 'Siber Güvenlik', cloud: 'Bulut', 'data-science': 'Veri Bilimleri' };
...
/**
 * Keep-alive: Render gibi platformlarda 15dk hareketsizlikte servis uykuya geçer.
 * Bu mekanizma sunucunun kendi kendini her ~9 dakikada bir (127.0.0.1:PORT) pingleyerek
 * uyanık kalmasını sağlar.
 *
 * ⚠ SINIRLAMA: Bu self keep-alive SADECE container AYAKTAYKEN çalışır.
 * Render starter plan'da container 15dk sonra TAMAMEN DURUR → self ping de durur.
 * Bu nedenle self keep-alive TEK BAŞINA YETERLİ DEĞİLDİR.
 *
 * ✅ HARİCİ ÇÖZÜMLER (biriyle birlikte kullanılmalıdır):
 *   - .github/workflows/keep-alive.yml → GitHub Actions ile her 8dk harici ping
 *   - UptimeRobot (ücretsiz) → 5dk aralıkla HTTP monitor
 *   - render.yaml → plan: starter_plus (ücretli, uyutmaz)
 *
 * DISABLE_KEEP_ALIVE=1 ile devre dışı bırakılabilir (örn: starter_plus plan'da gerekmez).
 */
function startKeepAlive(port, host, server) {
    ...
    const INTERVAL_MS = 9 * 60 * 1000 + 17 * 1000; // ~9 dk 17 sn — :00/:30 etrafındaki yığılmayı önler
    ...
    function doPing() {
        ...
        req.on('error', (err) => {
            logger.warn(`Keep-alive ping başarısız: ${err.message}`);
        });
    ...
    // İlk ping sunucu hazır olunca 30sn sonra atılsın
    const initialTimer = setTimeout(() => {
        doPing();
        setInterval(doPing, INTERVAL_MS);
        logger.info(`Keep-alive başlatıldı: her ~9dk health endpoint pinglenecek`);
    }, 30_000);

    // keep-alive timer'ı export et (test/graceful shutdown için)
    server._keepAliveInitialTimer = initialTimer;
}
